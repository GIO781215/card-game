...
# 牌卡代號 → 檔名（資料夾由 GameState.card_skin 決定）
...
func get_card_texture(code: String) -> Texture2D:
	return get_card_texture_for_skin(code, GameState.card_skin)

func get_card_texture_for_skin(code: String, skin: String) -> Texture2D:
	var filename: String = _CARD_IMAGE.get(code, "")
	if filename == "":
		return null
	var path := "res://assets/images/" + skin + "/" + filename
